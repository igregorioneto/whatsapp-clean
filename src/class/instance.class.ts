...
import { downloadMessage } from '../helpers/downloadMSG.helper';
import { generateVC } from '../helpers/genVC.helper';
import { useMongoDBAuthState } from '../helpers/mongoAuth.helper';
import { processButton } from '../helpers/processBTN.helper';
...
class WhatsAppInstance {
  socketConfig = {
    ...
    logger: pino({
      level: envorimentVariables.log.level,
    }),
    browser: [],
    auth: null,
  };
  ...
  axiosInstance = axios.create({
    baseURL: envorimentVariables.webhookUrl,
  });
  collection: any;
  ...
  async init() {
    this.collection = await mongoose.connect(envorimentVariables.mongoose.uri)
    const { state, saveCreds } = await useMongoDBAuthState(this.collection);
    this.authState = { state: state, saveCreds: saveCreds };
    this.socketConfig.auth = this.authState.state;
    this.socketConfig.browser = Object.values(envorimentVariables.browser);
    this.instance.sock =  makeWASocket(this.socketConfig as any);
    this.setHandler();
    return this;
  }
  ...
  setHandler() {
    ...
    sock?.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        if (
          ...
          await this.collection.drop().then((r) => {
            winstonLogger.info('STATE: Droped collection');
          });
        ...
        if (
          ['all', 'connection', 'connection.update', 'connection:close'].some(
            (e) => envorimentVariables.webhookAllowedEvents.includes(e),
          )
        )
        ...
        if (envorimentVariables.mongoose.enabled) {
      ...
      if (qr) {
        QRCode.toDataURL(qr).then((url) => {
          ...
          if (
            this.instance.qrRetry >= +envorimentVariables.instance.maxRetryQr
          ) {
            ...
            this.instance.qr = ' ';
            winstonLogger.info('socket connection terminated');
          }
        });
      }
    });
    ...
    sock?.ev.on('presence.update', async (json) => {
      if (
        ['all', 'presence', 'presence.update'].some((e) =>
          envorimentVariables.webhookAllowedEvents.includes(e),
        )
      )
    ...
    sock?.ev.on('messages.upsert', async (m) => {
      ...
      if (envorimentVariables.markMessagesRead) {
      ...
      m.messages.map(async (msg) => {
        ...
        if (envorimentVariables.webhookBase64) {
          switch (messageType) {
            case 'imageMessage':
    ...
    sock?.ev.on('group-participants.update', async (newChat) => {
      ...
      if (
        ...
        ].some((e) => envorimentVariables.webhookAllowedEvents.includes(e))
      )
        await this.SendWebhook(
  ...
  async deleteInstance(key) {
    try {
      ...
      winstonLogger.error('Error updating document failed');
    }
  }
  ...
  async getUserOrGroupById(id) {
    try {
      ...
      winstonLogger.error('Error get group failed');
    }
  }
  ...
  async updateDbGroupsParticipants() {
    try {
      let groups: Groups = await this.groupFetchAllParticipating();
      let Chats = await this.getChat();
      if (groups && Chats) {
      ...
      winstonLogger.error(e);
      winstonLogger.error('Error updating groups failed');
    }
  }
  ...
  async createNewGroup(name, users) {
    try {
      ...
      winstonLogger.error(e);
      winstonLogger.error('Error create new group failed');
    }
  }
  ...
  async leaveGroup(id) {
    try {
      ...
      winstonLogger.error(e);
      winstonLogger.error('Error leave group failed');
    }
  }
  ...
  async getInviteCodeGroup(id) {
    try {
      ...
      winstonLogger.error(e);
      winstonLogger.error('Error get invite group failed');
    }
  }
  ...
  async groupFetchAllParticipating() {
    try {
      ...
      winstonLogger.error('Error group fetch all participating failed');
    }
  }
  ...
  async readMessage(msgObj) {
    try {
      ...
      winstonLogger.error('Error read message failed');
    }
  }
  ...
  async reactMessage(id, key, emoji) {
    try {
      ...
      winstonLogger.error('Error react message failed');
    }
  }
}
